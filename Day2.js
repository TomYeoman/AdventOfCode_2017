// Create an array of lines from the input
const PUZZLE_INPUT = `179	2358	5197	867	163	4418	3135	5049	187	166	4682	5080	5541	172	4294	1397
...
PUZZLE_INPUT.forEach((currLine)=> {
    // Sanitise data, and convert the currLine into a int array
    numbersArr = currLine.replace(/\s/g, " ").split(' ');
    total += Math.abs(Math.min(...numbersArr)-Math.max(...numbersArr))
})
...
PUZZLE_INPUT.forEach((currLine)=> {
    // Sanitise data, and convert the currLine into a int array
    numbersArr = currLine.replace(/\s/g, " ").split(' ');
    numbersArr.map((currNum)=> {
        // Does currNum divide evenly with any other number on the line
        numbersArr.map((otherNum)=> {
            if (currNum % otherNum === 0 && currNum != otherNum){
                total += currNum / otherNum
            }
        }) 
    })
})
